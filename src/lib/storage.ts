  ...
  try {
    ...
    if (error) {
      ...
      if (error.message?.includes('Bucket not found') || error.message?.includes('bucket does not exist')) {
        alert(`❌ Bucket "${bucket}" belum dibuat di Supabase Storage!\n\nSilakan buat bucket terlebih dahulu di Supabase Dashboard:\n1. Buka https://supabase.com/dashboard/project/clfgsfdnyvdjgcevafzw/storage/buckets\n2. Klik "New Bucket"\n3. Nama bucket: ${bucket}\n4. Centang "Public bucket" agar foto bisa diakses publik\n5. Klik "Create bucket"`);
      } else {
